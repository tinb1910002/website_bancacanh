...
<?php
        if ($_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            $a = 1;
            $order = new order;
            $show_order_search1 = $order ->show_order_search1($name);
        }
        else{
            $a = 1;
            $order = new order;
            $show_order1 = $order ->show_order1();
        }
?>
            <div class="admin-content-right text-center" style="width: 100%; padding: 1rem 9%;">
                <div class="admin-content-right-cartegory-list">
                    <h1 class="text-danger">DANH SÁCH ĐƠN HÀNG</h1>
                    ...
                    <table class="table table-secondary" style="    margin-top: 70px;">
                        ...
                        <?php
                            if (isset($_POST['search_name'])) { 
                                $name = $_POST['keyword'];
                                if($show_order_search1){
                                    $i=0;
                                        while($result = $show_order_search1->fetch_assoc()){
                                            $i++
                        ?>
                                            <tr>
                                                <td><?php echo $i ++ ?></td>
                        ...
                        if($show_order1){
                            $i=0;
                                    while($result = $show_order1->fetch_assoc()){
                                        $i++;      
                        ?>
                                            <tr>
                                                <td><?php echo $a ++ ?></td>
